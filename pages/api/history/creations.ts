...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const base = `SELECT id, created_at, provider, kind, prompt, model, resolution, duration, expand_prompt, source_url, resource_url, thumb_url FROM saved_creations WHERE key_hash = ?`
  const rows = provider
